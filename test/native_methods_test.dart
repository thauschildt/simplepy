...
void runSimplePyTest(
  ...
  try {
    ...
    if (errorContains != null && !runtimeErrorMessage.contains(errorContains)) {
      fail(
        'Expected RuntimeError containing "$errorContains", but got: $runtimeErrorMessage\nCode:\n$code',
      );
    }
  } catch (e, stackTrace) {
  ...
  if (expectError &&
      errorContains != null &&
      !(caughtError! as RuntimeError).message.contains(errorContains)) {
    fail(
      'Expected RuntimeError containing "$errorContains", but got: "$caughtError"\nCode:\n$code',
    );
  }
  ...
  if (!expectError) {
    if (expectedOutput != null) {
      expect(
        output.toString(),
        expectedOutput,
        reason: 'Output mismatch for code:\n$code',
      );
    }
    if (expectedVariables != null && interpreter != null) {
      expectedVariables.forEach((varName, expectedValue) {
        ...
        expect(
          ...
          reason: 'Variable "$varName" mismatch for code:\n$code',
        );
      });
    }
  }
...
''', expectedOutput: "[2] 4 1 3\n");
      runSimplePyTest(
        'l=[].pop()',
        expectError: true,
        errorContains: 'IndexError: pop from empty list',
      );
      runSimplePyTest(
        'l=[1].pop(1)',
        expectError: true,
        errorContains: 'IndexError: pop index out of range',
      );
      runSimplePyTest(
        'l=[1].pop(-2)',
        expectError: true,
...
''',
      ...
      runSimplePyTest(
        "l=['a'].index('b')",
        expectError: true,
        errorContains: "ValueError: 'b' is not in list",
      );
      runSimplePyTest(
        "l=['a','b'].index('a', 1)",
        expectError: true,
        errorContains: "ValueError: 'a' is not in list",
      );
      runSimplePyTest(
        "l=['a'].index('a', [1])",
        expectError: true,
...
i5 = t_nested.index((1,2))
  ''',
      ...
      runSimplePyTest(
        "t=('a',).index('b')",
        expectError: true,
        errorContains: "ValueError: tuple.index(x): x not in tuple",
      );
      runSimplePyTest(
        "t=('a','b').index('a', 1)",
        expectError: true,
...
''',
      ...
      expect(
        ...
        isA<RuntimeError>().having(
          (e) => e.message,
          'message',
          contains("TypeError: unhashable type: 'list'"),
        ),
      );
    });
...
''', expectedOutput: "True\nTrue\nTrue\n");
      expect(
        runCode('set().pop()').error,
        isA<RuntimeError>().having(
          (e) => e.message,
          'message',
          contains("KeyError: 'pop from an empty set'"),
        ),
      );
    });
...
''',
      ...
      expect(
        runCode('s={1}.union(1)').error,
        isA<RuntimeError>().having(
          (e) => e.message,
          'message',
          contains('is not iterable'),
        ),
      );
      expect(
        runCode('s={1}.union([[]])').error,
        isA<RuntimeError>().having(
          (e) => e.message,
          'message',
          contains("unhashable type: 'list'"),
        ),
      );
    });
...
''',
      ...
      expect(
        runCode('s={1}.intersection(1)').error,
        isA<RuntimeError>().having(
          (e) => e.message,
          'message',
          contains('is not iterable'),
      ...
      expect(
        runCode('s={1}.intersection([[]])').error,
        isA<RuntimeError>().having(
          (e) => e.message,
          'message',
          contains("unhashable type: 'list'"),
...
''',
      ...
      expect(
        runCode('s={1}.difference(1)').error,
        isA<RuntimeError>().having(
          (e) => e.message,
          'message',
          contains('is not iterable'),
      ...
      expect(
        runCode('s={1}.difference([[]])').error,
        isA<RuntimeError>().having(
          (e) => e.message,
          'message',
          contains('unhashable type'),
        ),
      );
    });
...
''',
      ...
      expect(
        runCode('s={1}.isdisjoint(1)').error,
        isA<RuntimeError>().having(
          (e) => e.message,
          'message',
          contains('is not iterable'),
      ...
      expect(
        runCode('s={1}.isdisjoint([[]])').error,
        isA<RuntimeError>().having(
          (e) => e.message,
          'message',
          contains("unhashable type: 'list'"),
        ),
      ); // isdisjoint requires hashable
    });
...
''',
      ...
      expect(
        runCode('s={1}.issuperset(1)').error,
        isA<RuntimeError>().having(
          (e) => e.message,
          'message',
          contains('is not iterable'),
...
''',
      ...
      runSimplePyTest(
        "d={'a':1}.pop('b')",
        expectError: true,
        errorContains: "KeyError: 'b'",
      );
      runSimplePyTest(
        "{}.pop('a')",
        expectError: true,
        errorContains: "KeyError: 'a'",
      );
      runSimplePyTest(
        "d={'a':1}.pop([])",
        expectError: true,
        errorContains: "TypeError: unhashable type",
      );
      runSimplePyTest(
        "d={'a':1}.pop([], 'default')",
        expectError: true,
...
''',
      ...
      runSimplePyTest(
        "{'a':1}.update(123)",
        expectError: true,
        errorContains: "TypeError: 'int' object is not iterable",
      );
      runSimplePyTest(
        "{'a':1}.update([[['unhashable'] ,1]])",
        expectError: true,
        errorContains: "TypeError: unhashable type: 'list'",
      ); // Unhashable key
    });
...
''',
      ...
      runSimplePyTest(
        "s='abc'.find(1)",
        expectError: true,
        errorContains: 'TypeError: must be str',
      );
      runSimplePyTest(
        "s='abc'.find('a', 'b')",
        expectError: true,
...
''',
    ...
    test('str.split()', () {
      ...
      // Maxsplit
      runSimplePyTest(
        "l = 'a b c d'.split(None, 1)",
        expectedVariables: {
          'l': ['a', 'b c d'],
        },
      );
      runSimplePyTest(
        "l = ' a  b c d '.split(None, 2)",
        expectedVariables: {
          'l': ['a', 'b', 'c d'],
        },
      );
      ...
      runSimplePyTest(
        "'abc'.split('')",
        expectError: true,
        errorContains: 'ValueError: empty separator',
      );
    });
    ...
    test('str.join()', () {
      runSimplePyTest(
        "s = ','.join(['a', 'b', 'c'])",
        expectedVariables: {'s': 'a,b,c'},
      );
      runSimplePyTest(
        "s = ' '.join(['hello', 'world'])",
        expectedVariables: {'s': 'hello world'},
      );
      runSimplePyTest(
        "s = ''.join(['a', 'b', 'c'])",
        expectedVariables: {'s': 'abc'},
      );
      ...
      runSimplePyTest(
        "','.join([1, 2])",
        expectError: true,
        errorContains: 'TypeError: sequence item 0: expected str instance',
      );
      runSimplePyTest(
        "','.join({})",
        expectError: true,
        errorContains: 'TypeError: can only join an iterable of strings',
      ); // Non-iterable or wrong type
      runSimplePyTest(
        "123.join(['a'])",
        expectError: true,
        errorContains: "Undefined variable 'join'",
      );
    });
...
''',
      ...
      runSimplePyTest(
        "'abc'.startswith(1)",
        expectError: true,
        errorContains: 'TypeError: startswith first arg must be str',
      );
      runSimplePyTest(
        "'abc'.startswith('a', 'b')",
        expectError: true,
...
''',
      ...
      runSimplePyTest(
        "'abc'.endswith(1)",
        expectError: true,
        errorContains: 'TypeError: endswith first arg must be str',
      );
      runSimplePyTest(
        "'abc'.endswith('a', 'b')",
        expectError: true,
